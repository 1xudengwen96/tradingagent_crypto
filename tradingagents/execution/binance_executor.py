# tradingagents/execution/binance_executor.py
"""
Binance 合约执行层
解析投资组合管理者的文本决策，转换为 Binance API 可执行的合约交易指令。
支持 Sandbox 沙盒和实盘两种模式。
"""
...
class SignalParser:
    ...
    def _extract_leverage(cls, text: str) -> int:
        ...
        for p in patterns:
            ...
            if m:
                val = int(m.group(1))
                # Sanity-clamp to Binance's max perpetual leverage
...
class BinanceExecutor:
    """
    Translates a TradeSignal into live (or sandbox) Binance API calls.
    ...
    result = executor.execute(signal, symbol="BTC/USDT", capital_usdt=1000.0)
    """
    ...
    def __init__(
        ...
        ccxt_options = {
            'defaultType': 'future',
            'test': sandbox,  # Test network for sandbox
        }
        ...
        # Binance sandbox/testnet uses different URLs
        if sandbox:
            self._exchange.urls['api'] = {
                'public': 'https://testnet.binancefuture.com/fapi/v1',
                'private': 'https://testnet.binancefuture.com/fapi/v1',
            }
            self._exchange.urls['test'] = {
                'api': 'https://testnet.binancefuture.com/fapi/v1'
            }
            logger.info("BinanceExecutor running in SANDBOX mode (testnet.binancefuture.com)")
        else:
            logger.warning("BinanceExecutor running in LIVE mode — real funds at risk!")
    ...
    def execute(
        ...
        """
        Execute a TradeSignal on Binance perpetual futures.
        ...
        symbol : str
            e.g. "BTC/USDT" (Binance format, not "BTC/USDT:USDT")
        capital_usdt : float
        ...
        try:
            ...
            logger.error("Network error communicating with Binance: %s", e)
    ...
    def _open_position(
        ...
        if sl_price:
            try:
                # Binance uses stopLossPrice parameter for stop-loss orders
    ...
    def fetch_account_balance(self) -> dict:
        """Return USDT balance from futures account."""
        try:
