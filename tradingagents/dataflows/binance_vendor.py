# tradingagents/dataflows/binance_vendor.py
"""
Binance 数据层 —— 通过 CCXT 获取加密货币合约市场数据。
...
def _filter_closed_daily_candles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: 包含 datetime 列（UTC 时区感知）的 OHLCV DataFrame
...
# 交易所实例（单例缓存）
# ─────────────────────────────────────────────
_exchange_cache: Optional[ccxt.binanceusdm] = None


def get_binance_exchange() -> ccxt.binanceusdm:
    """获取（或创建）Binance 交易所实例，支持沙盒模式。"""
    global _exchange_cache
    if _exchange_cache is not None:
        return _exchange_cache

    config = get_config()
    # Binance USDM futures uses 'future' defaultType
    ccxt_options = {
        "defaultType": "future",
    }
    ...
    # ⚠️ 重要：Binance 测试网不支持 fapiPublic 公开数据端点
    # 所以即使沙盒模式，也使用实盘 API 获取公开数据（只读，不下单）
    # 沙盒模式只影响交易执行（通过 shadow_executor 模拟）
...
def get_crypto_ohlcv(symbol: str, timeframe: str = "1h", limit: int = 200) -> str:
    ...
    Args:
        symbol: 交易对，如 'BTC/USDT:USDT'（Binance 永续合约格式）
...
def get_open_interest(symbol: str) -> str:
    ...
    try:
        ...
        # Binance USDM provides open interest via public API
        # Use ccxt's fetch_open_interest if available
...
def get_crypto_global_news(limit: int = 20) -> str:
    ...
    try:
        ...
        posts = data.get("results", [])
        if not posts:
            return "No global crypto macro news available"
        ...
        if not macro_posts:
            return "=== Global Crypto Macro News ===\n\nNo major macro events detected in the latest news.\nMarket may be in a quiet period — suitable for technical trading."

        result = "=== Global Crypto Macro News (High-Impact Events) ===\n\n"
        result += "⚠️ ATTENTION: Macro events detected — consider reducing position size or staying neutral.\n\n"
        ...
        for i, post in enumerate(macro_posts, 1):
            ...
            currency_list = ", ".join([c.get("code", "") for c in currencies[:5]]) if currencies else "General"

            result += f"{i}. [{published}] {title}\n"
            result += f"   Source: {source} | Related: {currency_list}\n\n"

        return result

    except Exception as e:
        return f"Error fetching global crypto macro news: {str(e)}"
